...
async def test_mcp_lock():
    ...
    # 0. Ensure fresh start and singleton consistency
    ...
    # Use 'throttle' which exists in the mock config
    result = await handle_call_tool("write_channel", {"channel_id": "throttle", "value": 10.0})
    ...
    result = await handle_call_tool("write_channels", {"writes": [{"channel_id": "throttle", "value": 10.0}]})
    print(f"Result: {result[0].text}")
    assert "Error: Cannot perform manual write" in result[0].text
    ...
    result = await handle_call_tool("write_channel", {"channel_id": "throttle", "value": 1.0})
    print(f"Result: {result[0].text}")
